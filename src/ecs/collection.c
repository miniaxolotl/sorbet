...
bool collection__register_entity(COLLECTION_T *collection, ENTITY_T *entity)
{
  ...
  if (queue_empty(collection->id_pool) && (collection->id_count < ECS__ENTITIES_MAX))
  {
                      ...
                      SDL_Event *event, SORBET_LENGTH_T delta)
{
  ...
  {
    system = collection->system_list->data[i];
    if (!system || system->status != ENABLED)
    {
      continue;
    }
...
void collection__destroy(COLLECTION_T *collection)
{
  ...
  collection__flush_components(collection);

  for (size_t i = 0; i < collection->system_list->used; i++)
  {
    SYSTEM_T *sys = (SYSTEM_T *)collection->system_list->data[i];
    if (sys)
      system__destroy(sys);
  }
  vector__free(collection->system_list);

  for (size_t i = 0; i < collection->entity_list->used; i++)
  {
    ENTITY_T *ent = (ENTITY_T *)collection->entity_list->data[i];
    if (ent)
      entity__destroy(ent);
  }
  vector__free(collection->entity_list);

  if (collection->id_pool)
  {
    while (!queue_empty(collection->id_pool))
    {
      ID_T *id = (ID_T *)queue_pop(collection->id_pool);
      free(id);
    }
    free(collection->id_pool->data);
    free(collection->id_pool);
  }
